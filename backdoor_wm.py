...
def train_and_evaluate_all(runs=3):
    ...
    for dataset_name in dataset_names:
        ...
        for model_name, model_class in model_classes.items():
            for run in range(runs):
                ...
                data, trigger_nodes = inject_backdoor_trigger(data.clone(), trigger_rate=0.01, l=20, target_label=0)
                ...
                wm_pred = outputs['hard_label'][trigger_nodes]
                wm_acc = (wm_pred == 0).float().mean().item()
    ...
    df.to_csv(f'backdoorwm_allmodels_results_{timestamp}.csv', index=False)
    summary.to_csv(f'backdoorwm_allmodels_summary_{timestamp}.csv', index=False)
