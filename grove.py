...
def train_and_evaluate_all(runs=3):
    ...
    df.to_csv(f'clean_baseline_allmodels_results_{timestamp}.csv', index=False)
    summary.to_csv(f'clean_baseline_allmodels_summary_{timestamp}.csv', index=False)
    print("🎉 All clean baseline training and evaluation complete.")
